...
@section('content')
<h1 class="text-3xl font-bold mb-6">Projeyi Düzenle</h1>
...
<form action="{{ route('admin.projects.update', $project->id) }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded shadow max-w-2xl">
    @csrf
    @method('PUT')
    <div class="mb-4">
    ...
    <div class="mb-4">
        ...
        <textarea name="description" id="description" rows="5" required
            class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 @error('description') border-red-500 @enderror">{{ old('description', $project->description) }}</textarea>
    </div>
    <div class="mb-4">
        ...
        @if($project->image)
            <div class="mt-3">
                <img src="{{ asset('storage/' . $project->image) }}" alt="{{ $project->title }}" class="max-w-xs rounded shadow">
            </div>
        @endif
    </div>
    <div class="mb-4">
    ...
    <div class="flex items-center">
        ...
        <input type="checkbox" name="is_active" value="1" id="is_active" class="mr-2" {{ old('is_active', $project->is_active ?? 0) ? 'checked' : '' }}>
