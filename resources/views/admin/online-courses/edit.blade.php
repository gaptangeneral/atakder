...
@section('title', 'Online Kurs Bilgisini Düzenle')

@section('content')
<h1 class="text-3xl font-bold mb-6">Online Kurs Bilgisini Düzenle</h1>
...
<form action="{{ route('admin.online-courses.update', $onlineCourse->id) }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded shadow max-w-2xl">
    @csrf
    @method('PUT')
    <div class="mb-4">
        <label for="title" class="block font-semibold mb-1">Başlık *</label>
        <input type="text" name="title" id="title" value="{{ old('title', $onlineCourse->title) }}" required
    ...
    <div class="mb-4">
        ...
        <textarea name="content" id="content" rows="10" required
            class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 @error('content') border-red-500 @enderror">{{ old('content', $onlineCourse->content) }}</textarea>
    </div>
    <div class="mb-4">
        ...
        @if($onlineCourse->image)
            <div class="mt-3">
                <img src="{{ asset('storage/' . $onlineCourse->image) }}" alt="{{ $onlineCourse->title }}" class="max-w-xs rounded shadow">
            </div>
        @endif
    </div>
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        <div>
            ...
            <input type="text" name="duration" id="duration" value="{{ old('duration', $onlineCourse->duration) }}" placeholder="Örn: 4 Hafta"
        ...
        <div>
            <div class="flex items-center mt-6">
                ...
                <input type="checkbox" name="has_certificate" value="1" id="has_certificate" class="mr-2" {{ old('has_certificate', $onlineCourse->has_certificate) ? 'checked' : '' }}>
    ...
    <div class="mb-4">
        <div class="flex items-center">
            ...
            <input type="checkbox" name="is_active" value="1" id="is_active" class="mr-2" {{ old('is_active', $onlineCourse->is_active) ? 'checked' : '' }}>
